    ...
    for(i = 1 ; i <= PNUM ; i++){
        ...
        if(pid == 0){ // 자식 프로세스
			...
			*/
            ...
            // [Print] 및 [Terminated] 출력 로직은 그대로 유지합니다.
            
            // NOTE: 이 코드는 부하 생성 및 weight 부여에만 사용됩니다.
            // 최종 통계는 아래 부모 프로세스에서 추출합니다.
    ...
    for(i = 1 ; i <= PNUM ; i++) {
        ...
        if (terminated_pid > 0) {
            ...
            if (get_proc_stats(terminated_pid, &stats) == 0) {
                ...
                printf(1, "STAT| PID:%d, Weight:%d | AT:%d, CT:%d, CPUT:%d, FT:%d | TT:%d, WT:%d, RT:%d\n",
                       stats.pid, stats.weight, 
